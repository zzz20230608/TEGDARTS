...
parser.add_argument('--tegepochs', type=int, default=50, help='num of TEGNAS training epochs')
...
def train(train_queue, valid_queue, model, architect, criterion, optimizer, lr, te_reward_generator,baseline):
    ...
    for step, (input, target) in enumerate(train_queue):
        ...
        if step % args.report_freq == 0:
            ...
            logging.info('alphas_normal = %s', str(model.alphas_normal.cpu().detach().numpy()))
            logging.info('alphas_reduce = %s', str(model.alphas_reduce.cpu().detach().numpy()))
...
def continous_arch(arch_parameters):
    probs = [nn.functional.softmax(arch_parameters[0], dim=-1),
             nn.functional.softmax(arch_parameters[1], dim=-1)]
    LogSoftmax = nn.LogSoftmax(dim=-1)
    log_prob1 = LogSoftmax(arch_parameters[0]).reshape(-1)
    log_prob2 = LogSoftmax(arch_parameters[1]).reshape(-1)
    log_probs = torch.cat((log_prob1, log_prob2), dim=0)
