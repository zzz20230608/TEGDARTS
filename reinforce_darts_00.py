...
class Policy_DARTS(nn.Module):
    ...
    def __init__(self, max_nodes, search_space):
        ...
        self.arch_normal_parameters = self.arch_parameters[0]
        self.arch_reduce_parameters = self.arch_parameters[1]
        ...
        def _parse(weights):
            ...
            for i in range(self._steps):
                ...
                _edge_indice = sorted(range(i + 2), key=lambda x: -max(
                    W[x][k] for k in range(len(W[x])) if k != self.search_space.index('none')))[:2]
    ...
    def forward(self):
        alphas = [nn.functional.softmax(self.arch_parameters[0], dim=-1),
                  nn.functional.softmax(self.arch_parameters[1], dim=-1)]
        return alphas
        ...
        for cell_idx, action in enumerate(actions):
            ...
            for block_idx in range(1, 4):
                ...
                for edge in range(2 + block_idx):
                    print('-' * 50)
                    _logit.append(
                        policy.arch_parameters[cell_idx][edge + cum_edges, actions[cell_idx][edge + cum_edges]].item())
...
def main(xargs, nas_bench):
    ...
    xargs.save_dir = xargs.save_dir + \
                     "/LR%.2f-%s-buffer%d-batch%d-repeat%d" % (
                     xargs.learning_rate, xargs.init, xargs.te_buffer_size, xargs.batch_size, xargs.repeat) + \
    ...
    logger.log(
        '||||||| {:10s} ||||||| Train-Loader-Num={:}, Valid-Loader-Num={:}'.format(xargs.dataset, len(train_data),
                                                                                   len(valid_data)))
    ...
    te_reward_generator = Buffer_Reward_Generator(xargs, xargs.search_space_name, search_space, train_data, valid_data,
                                                  class_num)
    for _step in range(total_steps):
        ...
        if xargs.search_space_name == 'nas-bench-201':
            ...
            logger.writer.add_scalar("accuracy/derive", nas_bench.query_meta_info_by_index(
                nas_bench.query_index_by_arch(policy.genotype())).get_metrics(dataname, 'x-valid')['accuracy'],
                                     step_current)
            probs = policy()
            ...
            logger.writer.add_scalar("TE/Linear_Regions", te_reward_generator._buffers['region'][-1], step_current)
            logger.writer.add_scalar("TE/MSE", te_reward_generator._buffers['mse'][-1], step_current)
            logger.writer.add_scalar("reinforce/entropy",
                                     sum([-(torch.log(prob) * prob).sum(1).mean(0) for prob in probs]) / 2,
                                     step_current)
        ...
        optimizer.step()
        step_current += 1
        logger.log('step [{:3d}] : average-reward={:.3f} : policy_loss={:.4f} : {:}'.format(_step, baseline.value(),
                                                                                            policy_loss.item(),
                                                                                            policy.genotype()))
        if xargs.search_space_name == 'nas-bench-201':
...
if __name__ == '__main__':
    ...
    parser.add_argument('--track_running_stats', type=int, default=1, help='something wrong')
